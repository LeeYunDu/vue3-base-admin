<template>
  ...
  <el-button v-if="selectType==='button'" type="primary" size="small" @click="visible=true"> 选择人员</el-button>
  <selectedInput
  ...
  setup(props, ctx) {
    ...
    const initTreeChecked = () => {
      ...
      try {
        ...
        if (typeof props.ids === 'string') {
          checkedIds = props.ids.split(',')
        }
        // 当双向绑定为数组时
        if (Array.isArray(props.ids)) {
          checkedIds = props.ids
        }
    ...
    watch(() => that.checkedResult, (val) => {
      ...
      if (Array.isArray(val)) {
        ...
        // 根据传入的ids类型来判断抛出的数据类型是字符串还是数组
        ctx.emit('update:ids', typeof props.ids === 'string' ? emitObject.ids : emitObject.ids.split(',')) // 整体勾选的人员id,是字符串,并以,隔开
    ...
    const init = () => {
      ...
      if (props.labels && props.ids) {
        ...
        const ids = typeof props.ids === 'string' ? props.ids.split(',') : props.ids
        const arr = []
