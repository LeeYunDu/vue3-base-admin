    ...
    <el-table :data="fileList" style="width: 100%;" :show-header="false">
      <el-table-column prop="fileName" label="文件名">
        <template #default="scope">
          <el-button size="small" type="text" class="nameButton" :title="scope.row.fileName" @click="handleOpenFile(scope.row)"> <span>{{ scope.row.fileName }}</span> </el-button>
          <el-button v-if="previewButton(scope.row)" size="small" @click="handleOpenFile(scope.row)">预览</el-button>
          <el-button type="primary" size="small" class="button" @click="handleDownloadFile(scope.row)">下载</el-button>
          <template v-if="!isView">
            <el-button type="danger" size="small" @click="handleEntityFileDelete(scope.$index)">删除</el-button>
          </template>
          <!-- <el-button v-if="showpdfbtn&&!isPDf&&!scope.row.nonsupport" size="small" @click="handleOpenFile(pdfList)">查看pdf</el-button> -->
        </template>
      </el-table-column>
    </el-table>
  </div>
