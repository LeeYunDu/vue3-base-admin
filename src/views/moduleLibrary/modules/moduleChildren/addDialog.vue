...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const rule = reactive({
      ...
      phone: [{ required: false, validator: validatorPhone, trigger: 'blur' }],
      idCard: [{ required: false, validator: validatorIdCard, trigger: 'blur' }]
    })

    // 表单提交
    const submitForm = async(value) => {
      ElMessage.info('表单提交成功')
      ctx.emit('success', JSON.parse(JSON.stringify(ruleForm.value)));
